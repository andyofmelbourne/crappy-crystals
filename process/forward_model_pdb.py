...
def make_map_ccp4(pdbid, res=5.):
    ...
    output   = os.path.abspath(os.path.join(dirnam, pdbid + '_fmodel.ccp4'))
    ...
    # run the code 
    cmd = 'phenix.mtz2map ' + pdb_fnam+'.mtz ' + pdb_fnam + ' include_fmodel=True'
    print(cmd)
    p = Popen(shlex.split(cmd), cwd=dirnam)
    p.wait()
    
    print('#########################################')
    print('ccp4_fnam',pdb_fnam+'_fmodel.ccp4')
    return pdb_fnam+'_fmodel.ccp4', pdb_fnam
